...
const Index = () => {
  ...
  // For mobile: if only 1 listing, show 2 from the full sorted list
  let listingsToShow = sortedListings;
  if (isMobile && sortedListings.length === 1) {
    // Use the same sort logic on all listings
    const allSorted = [...mockListings].sort((a, b) => {
      const getSortRank = (listing) => {
        if (!listing.top3Prices || listing.top3Prices.length === 0) return 2;
        const todayIdx = listing.top3Prices.findIndex(p => p.is_today_listing);
        if (todayIdx === -1) return 2;
    ...
    listingsToShow = allSorted.slice(0, 2);
  }
      ...
      <div className="container mx-auto px-4 pb-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">
            {listingsToShow.length} Properties Found
          </h2>
          <ViewToggle viewMode={viewMode} onViewModeChange={setViewMode} />
        </div>
        {viewMode === "grid" ? (
          <ListingsGrid listings={listingsToShow} />
        ) : (
          <ListingsTable listings={listingsToShow} />
        )}
      </div>
    </div>
  );
